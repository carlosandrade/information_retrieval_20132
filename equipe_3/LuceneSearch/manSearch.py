...
def usage():
    print sys.argv[0], "[--format=<format string>] [--index=<index dir>] [--field] <field name> [--stats] <query...>"
    print "default index is found from MANDEX environment variable"

try:
    options, args = getopt(sys.argv[1:], '', ['format=', 'index=', 'field=', 'stats'])
except GetoptError:
    usage()
    sys.exit(2)
...
if stats:
    print >>sys.stderr, "Foram encontrados %d politico(s) (em %s) com a query '%s' :" %(len(scoreDocs), duration, query)
